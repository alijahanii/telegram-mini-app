"use client";

import { useState } from "react";
import { GiftDTO, Rarity, RARITY_META } from "@/lib/types";
import { formatToman, toPersianDigits, cn } from "@/lib/format";
import { Button } from "@/components/ui/Button";
import { Minus, Plus } from "lucide-react";
import { useCartStore } from "@/store/cart-store";
import { toast } from "sonner";
import { hapticImpact } from "@/lib/telegram";

export function GiftDetailClient({ gift }: { gift: GiftDTO }) {
  const [qty, setQty] = useState(1);
  const addItem = useCartStore((s) => s.addItem);
  const rarity = RARITY_META[gift.rarity as Rarity] ?? RARITY_META.common;
  const outOfStock = gift.stock <= 0;

  function handleAdd() {
    addItem({
      itemType: "gift",
      refId: gift.id,
      title: gift.name,
      subtitle: rarity.label,
      icon: gift.icon,
      imageUrl: gift.imageUrl,
      unitPrice: Number(gift.price),
      quantity: qty,
      maxStock: gift.stock,
    });
    hapticImpact("medium");
    toast.success(`${gift.name} به سبد خرید اضافه شد`);
  }

  return (
    <div className="pb-[calc(100px+env(safe-area-inset-bottom))]">
      <div className={cn("relative mb-4 flex h-64 items-center justify-center overflow-hidden rounded-[24px] bg-gradient-to-br text-9xl", rarity.color)}>
        <span className="drop-shadow-[0_10px_24px_rgba(0,0,0,0.4)]">{gift.icon}</span>
        <span className={cn("absolute right-3 top-3 rounded-full bg-gradient-to-l px-3 py-1 text-xs font-bold text-white shadow", rarity.color)}>
          {rarity.label}
        </span>
      </div>

      <h2 className="mb-2 text-lg font-extrabold text-white">{gift.name}</h2>
      <p className="mb-4 text-sm leading-6 text-slate-400">{gift.description}</p>

      <div className="mb-4 flex items-center justify-between rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
        <div>
          <p className="mb-1 text-[11px] text-slate-400">قیمت</p>
          <p className="text-xl font-extrabold text-sky-400">{formatToman(gift.price)}</p>
        </div>
        <div className="text-left">
          <p className="mb-1 text-[11px] text-slate-400">موجودی</p>
          <p className={`text-sm font-bold ${outOfStock ? "text-red-400" : "text-emerald-400"}`}>
            {outOfStock ? "ناموجود" : `${toPersianDigits(gift.stock)} عدد`}
          </p>
        </div>
      </div>

      <div className="mb-4 flex items-center justify-between rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3">
        <span className="text-sm text-slate-300">تعداد</span>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setQty((q) => Math.max(1, q - 1))}
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 active:scale-90"
          >
            <Minus className="h-4 w-4" />
          </button>
          <span className="w-6 text-center font-bold">{toPersianDigits(qty)}</span>
          <button
            onClick={() => setQty((q) => Math.min(gift.stock || 1, q + 1))}
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 active:scale-90"
          >
            <Plus className="h-4 w-4" />
          </button>
        </div>
      </div>

      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-[#0a0e14]/95 p-4 backdrop-blur-xl pb-[calc(1rem+env(safe-area-inset-bottom))]">
        <div className="mx-auto flex max-w-lg items-center gap-3">
          <div className="flex-1">
            <p className="text-[11px] text-slate-400">مبلغ کل</p>
            <p className="text-base font-extrabold text-white">{formatToman(Number(gift.price) * qty)}</p>
          </div>
          <Button size="lg" disabled={outOfStock} onClick={handleAdd} className="flex-1">
            افزودن به سبد خرید
          </Button>
        </div>
      </div>
    </div>
  );
}
